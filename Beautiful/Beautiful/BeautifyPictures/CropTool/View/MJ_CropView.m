...
@interface MJ_CropView ()
<
UIScrollViewDelegate,
UIGestureRecognizerDelegate,
MJ_CropRectViewDelegate
>
...
@property (nonatomic, strong) MJ_CropRectView *cropRectView;
...
- (void)commonInit {
    ...
    self.cropRectView = [[MJ_CropRectView alloc] init];
...
- (BOOL)userHasModifiedCropArea {
    ...
    return (!CGPointEqualToPoint(zoomedCropRect.origin, CGPointZero) || !CGSizeEqualToSize(zoomedCropRect.size, self.image.size) || !CGAffineTransformEqualToTransform(self.rotation, CGAffineTransformIdentity));
}
// 旋转
...
// 可见与不可见视图
- (CGRect)cappedCropRectInImageRectWithCropRectView:(MJ_CropRectView *)cropRectView {
    ...
    if (CGRectGetMaxY(rect) > CGRectGetMaxY(self.zoomingView.frame)) {
        ...
        cropRect.size = CGSizeMake(!self.keepingCropAspectRatio ? cropRect.size.width : cropRect.size.width * (cappedHeight / cropRect.size.height), cappedHeight);
    }
